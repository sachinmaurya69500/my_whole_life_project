...
function renderReadOnlyProjectsTable(currentProject) {
	...
	relatedProjects.forEach((project) => {
		...
		tr.innerHTML = `
			<td class="py-3 pr-2">
				<span class="text-slate-200 cursor-pointer hover:text-cyan-300 transition" style="text-decoration: underline;">${escapeHtml(project.title)}</span>
			</td>
		...
		const titleSpan = tr.querySelector('span');
